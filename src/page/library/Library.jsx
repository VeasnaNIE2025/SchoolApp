...
//     downloadUrl: '/PDF/Library/MiniBotCamMindPlus/Mini BotCam with Mind Plus.pdf',
//     size: '6.1MB',
//     downloads: 25,
//   },
//   {
...
//   {
//     id: 7,
//     title: 'ការជួសជុល ការថែទាំ និងការតបណ្ដាញ',
//     author: 'វិទ្យាស្ថានជាតិអប់រំ',
//     category: 'បច្ចេកវិទ្យា',
//     cover: '/PDF/Library/Maintenent/Maintenent.jpg',
//     downloadUrl: '/PDF/Library/Maintenent/Maintenent.pdf',
//     size: '8.4MB',
//     downloads: 56,
//   },
// ]
...
//               {/* PDF Preview: ខាងឆ្វេង, បត់បែនតាមទំហំ */}
...
//                {/* Book Metadata: ខាងស្តាំ, ជាប់ខាងលើ (sticky) */}
//                 <div className="w-full lg:w-96 shrink-0 order-1 lg:order-2 lg:sticky lg:top-6">
...
const BOOKS = [
  ...
  {
    // ⚠️ FIX: id ដើមស្ទួនគ្នាជាមួយ "React js from 0 To Hero" (ទាំងពីរប្រើ id: 7)
    // នេះជាមូលហេតុចម្បងដែលបញ្ជីសៀវភៅបង្ហាញមិនស្ថិតស្ថេរ (ចេញ/ចូល ខុសពេល)
    // ព្រោះ React ច្រឡំធាតុទាំងពីរនេះថាជាធាតុតែមួយ
    id: 8,
    title: 'ការជួសជុល ការថែទាំ និងការតបណ្ដាញ',
    author: 'វិទ្យាស្ថានជាតិអប់រំ',
    category: 'បច្ចេកវិទ្យា',
...
// ------------------------------------------------------------------
// Sub-component: កាតសៀវភៅមួយ (ប្រើរួមគ្នាទាំង grid-by-category និង grid-search)
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
// Sub-component: ក្រុមសៀវភៅមួយប្រភេទ (ប្រើនៅទំព័រដើម)
// ------------------------------------------------------------------
function CategorySection({ category, books, onSelect, onSeeAll }) {
  return (
...
// ------------------------------------------------------------------
// Sub-component: Grid ធម្មតា (ប្រើនៅពេលស្វែងរក ឬជ្រើសរើសប្រភេទ)
// ------------------------------------------------------------------
function BookGrid({ title, books, onSelect }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200/50 dark:border-slate-800/80 pb-3">
        <h2 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
      ...
      {books.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-[#121E36] rounded-3xl border border-slate-200/40 dark:border-slate-800/60">
          <p className="font-khmer font-bold text-xl text-slate-500 dark:text-slate-400 mb-2">
            រកមិនឃើញសៀវភៅទេ
          </p>
          <p className="text-slate-400 text-sm">
            សាកល្បងស្វែងរកដោយប្រើពាក្យគន្លឹះផ្សេង ឬជ្រើសរើសប្រភេទផ្សេង
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
...
// ------------------------------------------------------------------
// Sub-component: ទំព័រព័ត៌មានលម្អិតសៀវភៅ
// ------------------------------------------------------------------
function BookDetail({ book, onBack, onSelectCategory }) {
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: book.title, url: window.location.href }).catch((err) => console.log(err))
    } else {
      navigator.clipboard.writeText(window.location.href)
      alert('តំណភ្ជាប់ត្រូវបានចម្លងទុក!')
    }
  ...
  return (
    <div className="space-y-6">
      {/* Breadcrumbs */}
      <div className="flex items-center flex-wrap gap-2 text-xs md:text-sm text-slate-500 dark:text-slate-400 font-medium">
        <button onClick={onBack} className="hover:text-indigo-600 dark:hover:text-sky-400 transition-colors">
          ទំព័រដើម
        </button>
        <span className="text-slate-400 dark:text-slate-600">&gt;</span>
        <button
        ...
        <span className="text-slate-400 dark:text-slate-600">&gt;</span>
        <span className="text-slate-800 dark:text-slate-200 line-clamp-1">{book.title}</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        {/* PDF Preview */}
        <div className="w-full lg:flex-1 space-y-4 order-2 lg:order-1">
          <PdfPreview fileUrl={book.downloadUrl} fileName={book.title + '.pdf'} />
        </div>
        ...
        <div className="w-full lg:w-96 shrink-0 order-1 lg:order-2 lg:sticky lg:top-6">
          <div className="bg-white dark:bg-[#121E36] border border-slate-200/40 dark:border-slate-800/60 rounded-3xl p-6 shadow-sm">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
              និពន្ធដោយ: <span className="font-semibold text-slate-700 dark:text-slate-300">{book.author}</span>
            </p>
            ...
            <div className="bg-slate-100 dark:bg-[#0B1329]/40 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-4 flex flex-col gap-3.5">
              <div className="flex justify-between text-sm py-1.5 border-b border-slate-200/60 dark:border-slate-800 last:border-0">
                <span className="text-slate-500 dark:text-slate-400">ទំហំឯកសារ (Size)</span>
                <span className="font-bold text-slate-800 dark:text-slate-200">{book.size || '1.3 MB'}</span>
              </div>
              <div className="flex justify-between text-sm py-1.5 border-b border-slate-200/60 dark:border-slate-800 last:border-0">
                <span className="text-slate-500 dark:text-slate-400">ចំនួនទាញយក (Downloads)</span>
                <span className="font-bold text-sky-500 dark:text-sky-400 flex items-center gap-1">
                  <FiDownload className="w-3.5 h-3.5" /> {book.downloads || 0}
                </span>
              </div>
              <div className="flex justify-between text-sm py-1.5 border-b border-slate-200/60 dark:border-slate-800 last:border-0">
                <span className="text-slate-500 dark:text-slate-400">ទម្រង់ (Format)</span>
...
function Library() {
  ...
  const availableCategories = useMemo(
    () => CATEGORIES.filter((cat) => cat !== 'ទាំងអស់' && BOOKS.some((b) => b.category === cat)),
    []
  )

  const showByCategory = activeCategory === 'ទាំងអស់' && !query
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1329] font-khmer text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        {selectedBook ? (
          <BookDetail book={selectedBook} onBack={goHome} onSelectCategory={selectCategory} />
        ) : showByCategory ? (
          <div className="space-y-12">
          ...
          <BookGrid
            title={`${activeCategory}${query ? ` (ស្វែងរក: "${query}")` : ''}`}
            books={filteredBooks}
            onSelect={setSelectedBook}
          />
        )}
